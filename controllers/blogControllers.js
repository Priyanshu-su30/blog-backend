...
function fetchSingleBlog(req,res) {
    ...
    myBlogModel.findById(blogId.id).then((data)=>{
        console.log(data);
...
function deleteBlog(req,res){
    ...
    if (Object.hasOwn(blogId, "id")===false && blogId.id.length===0){
        return res.result(404).send({message: "You forgot to tell which blog to delete"})
    }
...
function updateBlog(req, res) {
    ...
    if (Object.hasOwn(blogId, "id")===false && blogId.id.length===0){
        return res.result(404).send({message: "You forgot to tell which blog to update"})
    }

    if(blogImage !== undefined){
        cloudinary.v2.uploader.upload_stream((error, result) =>{
            if(error !== undefined){
                return res.status(500).send({message:"Server failed to uplaod image"})
            };
...
function newBlog(req, res){
    ...
    cloudinary.v2.uploader.upload_stream((error, result) =>{
        ...
        newBlog.save().then(() => {
            return res.status(500).send({message:"Bravo, for new blog."})
        }).catch(()=>{
...
const obj = {newBlog, updateBlog, deleteBlog, getAllBlogs, userSpecificBlogs, fetchSingleBlog}
module.exports = obj;